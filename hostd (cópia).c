...
bool complete()
{
    ...
    if (!input_queue && !real_time_queue && !user_job_queue && !priority_one_queue && !priority_two_queue && !priority_three_queue && !current_process)
    {
        output = div(total, qtd);
        // printf("%d\n",total);
        // printf("%d\n",timer);
        //  printf("%d\n",qtd);
        // output1=div(timer,180);
        // if(output1.rem==0){
        printf("%d - %d\n", qtd, output.quot);
        // }
    }
    return (!input_queue && !real_time_queue && !user_job_queue && !priority_one_queue && !priority_two_queue && !priority_three_queue && !current_process);
}
...
void handle_current_process()
{
    ...
    else // The process hasn't finished
    {
            ...
            (current_process->priority != 0)) // There are still jobs in the queue(s)
        {
            ...
            // tempo ocioso = timer - pcb.instanteDeCPU.

            // timeLimit = tempo limite definido para o dispatch
            // que trata dos processos que deixaram de usar a cpu por muito tempo

            /*
            A logica abaixo será feita a cada tick, nas filas de prioridade 3 e 2.

                Se tempo ocioso de um processo é maior que timeLimit então
                    se é um processo novo(nunca foi usado) então
                        aumente a prioridade para 1
                    senão
                        aumente a prioridade um nivel acima

            */

            struct pcb *pcbIncognita = NULL;

            if (priority_three_queue) // A fila de prioridade 3 está vazia?
            {
                unsigned int diffTime = timer - priority_three_queue->instanteDeCPU;
                if (diffTime > timerLimit)
                {
                    //Retire o processo da fila de prioridade 3
                    pcbIncognita = priority_three_queue;
                    priority_three_queue = dequeue_pcb(priority_three_queue);

                    //Verifique o tipo do processo
                    if (pcbIncognita->pid == 0) // Mande para a fila de prioridade 1
                    {
                        priority_one_queue = enqueue_pcb(priority_one_queue, pcbIncognita);
                    }
                    else // Mande para a fila de prioridade um nivel acima
                    {
                        priority_two_queue = enqueue_pcb(priority_two_queue, pcbIncognita);
                    }
                }
                else
                {
            ...
            // Caso não tenha ninguém na fila de prioridade 3
            // Ou o cabeça da fila já passou recentemente pela CPU
            if (!pcbIncognita)
            {
                if (priority_two_queue) // a fila de prioridade 2 está vazia?
                {
                    unsigned int diffTime = timer - priority_two_queue->instanteDeCPU;
                    if (diffTime > timerLimit)
                    {
                        // Retire o processo da fila de prioridade 2
                        pcbIncognita = priority_two_queue;
                        priority_two_queue = dequeue_pcb(priority_two_queue);

                        // Mande para fila de prioridade 1
                        priority_one_queue = enqueue_pcb(priority_one_queue, pcbIncognita);
                    }
                }
            }
